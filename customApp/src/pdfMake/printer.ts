...
const TraversalTracker: any = require('./libs/traversalTracker');
const DocPreprocessor: any = require('./libs/docPreprocessor');
const DocMeasure: any = require('./libs/docMeasure');
const DocumentContext: any = require('./libs/documentContext');
const PageElementWriter: any = require('./libs/pageElementWriter');
const ColumnCalculator: any = require('./libs/columnCalculator');
const TableProcessor: any = require('./libs/tableProcessor');
const Line: any = require('./libs/line');
const pack: any = require('./libs/pack');
import { offsetVector } from './libs/offsetVector';
const fontStringify: any = require('./libs/fontStringify');
const isFunction: any = require('./libs/isFunction');
const TextTools: any = require('./libs/textTools');
const StyleContextStack: any = require('./libsstyleContextStack');
...
LayoutBuilder.prototype.layoutDocument = function (docStructure, fontProvider, styleDictionary, defaultStyle, background, header, footer, images, watermark, pageBreakBeforeFct) {
	...
	function addPageBreaksIfNecessary(linearNodeList, pages) {
		...
		linearNodeList = _.reject(linearNodeList, function (node: any) {
			return _.isEmpty(node.positions);
		});

		_.each(linearNodeList, function (node: any) {
			var nodeInfo: any = _.pick(node, [
		...
		return _.some(linearNodeList, function (node: any, index, followingNodeList) {
			if (node.pageBreak !== 'before' && !node.pageBreakCalculated) {
				node.pageBreakCalculated = true;
				var pageNumber: any = _.first(node.nodeInfo.pageNumbers);

				var followingNodesOnPage = _.chain(followingNodeList).drop(index + 1).filter(function (node0: any) {
					return _.includes(node0.nodeInfo.pageNumbers, pageNumber);
				}).value();

				var nodesOnNextPage = _.chain(followingNodeList).drop(index + 1).filter(function (node0: any) {
					return _.includes(node0.nodeInfo.pageNumbers, pageNumber + 1);
				}).value();

				var previousNodesOnPage = _.chain(followingNodeList).take(index).filter(function (node0: any) {
					return _.includes(node0.nodeInfo.pageNumbers, pageNumber);
				}).value();
	...
	function resetXYs(result) {
		_.each(result.linearNodeList, function (node: any) {
			node.resetXY();
		});
	}
...
function decorateNode(node) {
	...
	_.each(node.canvas, function (vector: any) {
    var x = vector.x,
    y = vector.y,
    x1 = vector.x1,
    y1 = vector.y1,
